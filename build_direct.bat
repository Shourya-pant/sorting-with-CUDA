...
echo.
echo Compiling with command: nvcc -I./include -o build/gpu_sorting src/main.cpp src/sort.cu -std=c++14 -ccbin="C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\14.36.32532\bin\Hostx64\x64"
echo.

REM First attempt - with standard location path
...
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo First compilation attempt failed, will try to compile CPU-only version
    echo.
